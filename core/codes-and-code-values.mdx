...
`exception/` declares three typed exceptions:
...
These derive from the standard `Abstract*` base classes in `infrastructure/core/exception/` (see [Infrastructure Core](/core/infrastructure-core)). FK violations from deleting an in-use code value surface as the generic `PlatformDataIntegrityException`.
