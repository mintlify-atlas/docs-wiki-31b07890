...
Apache Fineract exposes a large set of named runtime flags and tunables (~68 string constants in `GlobalConfigurationConstants`) that operators flip without redeploying — maker-checker, business date enablement, S3 document storage, accrual behaviour, password rules, COB toggles and many more. The `infrastructure/configuration/` package in `fineract-core` defines the entity, repository, constants, and service interface for these flags. This page tours the model and enumerates the notable settings.
...
The full list is in the file, ~68 constants long.
...
public interface ConfigurationDomainService {
    ...
    // ... ~70 typed methods total, one per flag
}
```
...
- `GlobalConfigurationRepository` extends Spring Data `JpaRepository<GlobalConfigurationProperty, Long>` with a `findOneByName(String name)` finder.
- `GlobalConfigurationRepositoryWrapper` decorates the repo with `findOneByNameWithNotFoundDetection(String propertyName)` and `findOneWithNotFoundDetection(Long configId)` that throw `GlobalConfigurationPropertyNotFoundException` when the row is absent. Application code typically depends on the wrapper.
...
<CardGroup cols={2}>
  ...
  <Card title="api/GlobalConfigurationConstants" icon="hashtag">
    ~68 string constants — one per flag.
  </Card>
