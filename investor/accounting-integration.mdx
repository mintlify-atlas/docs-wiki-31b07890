...
```mermaid
flowchart LR
    Step[LoanAccountOwnerTransferBusinessStep] --> Poster[LoanJournalEntryPoster<br/>fineract-portfolio]
...
`LoanJournalEntryPoster.postJournalEntriesForExternalOwnerTransfer(loan, transfer, previousOwner)` is the entry point the COB step actually calls; it lives in `fineract-portfolio` and dispatches to `AccountingService` (the investor-module interface) based on the transfer's status — `ACTIVE` or `ACTIVE_INTERMEDIATE` → `createJournalEntriesForSaleAssetTransfer`; `BUYBACK` or `BUYBACK_INTERMEDIATE` → `createJournalEntriesForBuybackAssetTransfer`.
