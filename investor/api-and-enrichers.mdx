...
This is the workhorse resource. It exposes the *entire transfer lifecycle*, the *owner directory*, and a *search* endpoint. There are eleven routes, grouped into five concerns.
...
`ExternalAssetOwnerSearchRequest` (under `service/search/domain/`) carries a free-text `text` field plus two date-range pairs — `submittedFromDate`/`submittedToDate` (the request creation window) and `effectiveFromDate`/`effectiveToDate` (the validity window). The actual SQL lives in `SearchingExternalAssetOwnerRepositoryImpl`.
