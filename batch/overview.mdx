...
are skipped and a synthetic response is emitted in their place carrying the
parent's failure status code and a `"Parent request with id N was
erroneous!"` body.
...
<Tabs>
  <Tab title="enclosingTransaction=false (default)">
    ...
    are still short-circuited — they receive a response with the parent's
    status code and a body of
