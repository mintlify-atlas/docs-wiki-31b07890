...
| `statusCode` | `Integer` | The HTTP status the underlying handler would have returned. `200` on success, `409` for skipped children when their parent fails, `500` for unexpected exceptions. |
...
The request body comes off the wire as a JSON array. JAX-RS hands it to the
resource as `List<BatchRequest>` via Jackson; for code paths that need to
re-parse the string,
...
Two separate JSON stacks coexist deliberately: the inbound array is parsed
once, while the per-request body field stays a `String` until a handler (or
the JsonPath resolver) needs to look inside it. That avoids re-serializing
deeply nested loan-application bodies and lets each handler choose its own
parser.
...
sequenceDiagram
    ...
    loop each root (depth-first)
        ...
        alt response 200
            ...
            Svc->>Svc: parentRequestFailedRecursive (409 skip)
        end
    end
...
<Note>
  ...
  failing parent are still skipped, with a synthetic `409` body of
