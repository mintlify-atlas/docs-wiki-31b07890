...
The `allprojects { ... }` block in `build.gradle` is where Fineract's "build convention plugin" effectively lives. There is no separate `convention-plugin/` directory under `buildSrc/`; instead, the conventions are baked into the root build script and applied to every subproject. The most important ones:
