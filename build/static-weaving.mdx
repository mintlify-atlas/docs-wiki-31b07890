...
Because `static-weaving.gradle` is applied to every subproject and acts only when the descriptor is present, the "list of modules that opt in" is whichever Java subprojects ship a `persistence.xml` at the well-known path. At the time of writing, a `git ls-files '**/jpa/static-weaving/module/**/persistence.xml'` returns 15 descriptors covering the domain layer plus the Spring Boot runtime:
...
- `fineract-provider` — owns its own descriptor because it carries a handful of provider-specific entities (jobs, scheduler, tenant, audit) on top of the entities pulled in transitively from the domain JARs.
...
- `fineract-security`, `fineract-validation`, `fineract-document`, `fineract-mix` — modules that ship logic / utilities but no `@Entity` types of their own (their entities live in `fineract-core` or `fineract-provider`).
- `fineract-command`, `fineract-command-async`, `fineract-command-disruptor`, `fineract-command-test`, `fineract-command-jdbc`, `fineract-command-audit` — the command-bus modules.
...
The runtime persistence unit used by Spring Boot at startup is a separate descriptor; the build-time descriptors are only ever read by `StaticWeave`.
