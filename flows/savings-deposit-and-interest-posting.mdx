...
| `WITHDRAWAL` | `WithdrawSavingsAccountCommandHandler` | `withdrawal(...)` |
...
| 5 | Poster | `fineract-savings/src/main/java/org/apache/fineract/portfolio/savings/service/SavingsSchedularInterestPoster.java` | Loops over the assigned `SavingsAccountData` and calls `postInterest(account, ...)` on the write service. |
...
| --- | --- |
| Handler | `WithdrawSavingsAccountCommandHandler` (`@CommandType(entity="SAVINGSACCOUNT", action="WITHDRAWAL")`). |
