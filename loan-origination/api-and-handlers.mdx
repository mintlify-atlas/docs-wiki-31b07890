...
`attachLoanOriginator(loanId, originatorId)` builds a command where:

- `entity = "LOAN_ORIGINATOR"`
- `action = "ATTACH"`
- `entityId = loanId` (via `withLoanId`)
- `subentityId = originatorId`

The handler uses both ids — `command.getLoanId()` and `command.subentityId()`.
