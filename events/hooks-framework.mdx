...
(which builds a Retrofit client). `ProcessorHelper` opt-in trusts self-signed certs based on a
hook configuration field, which is convenient for staging but should not be used in production.
