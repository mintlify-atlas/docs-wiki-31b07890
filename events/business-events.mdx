...
**loan account** and **loan transaction** events (`LoanCreateBusinessEvent`,
`LoanApprovedBusinessEvent`, `LoanDisbursalBusinessEvent`, `LoanRefundBusinessEvent`,
`LoanChargeOffBusinessEvent`, `LoanChargeAddedBusinessEvent`,
`LoanInstallmentLevelDelinquencyEvent`, the delinquency-range change events, and so on) lives in
sibling modules and is wired up by the serializers in
`event/external/service/serialization/serializer/loan/`. The presence of a
`*BusinessEventSerializer` in that folder is the canonical way to discover the full set without
walking every domain module.
