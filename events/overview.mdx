...
```java
businessEventNotifierService.notifyPreBusinessEvent(new LoanCreatedBusinessEvent(loan));
...
The concrete event classes — `LoanCreatedBusinessEvent`, `LoanApprovedBusinessEvent`,
...
`m_hook_registered_events` for any active hooks subscribed to that pair and dispatches the JSON payload
