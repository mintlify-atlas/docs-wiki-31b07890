...
```json
{
  "externalEventConfiguration": [
    ...
    { "type": "LoanCreatedBusinessEvent",    "enabled": false },
...
in `c_configuration` under the key `purge-external-events-older-than-days` (the constant
`GlobalConfigurationConstants.PURGE_EXTERNAL_EVENTS_OLDER_THAN_DAYS`). You can change it through
...
curl -X PUT https://fineract.example.com/fineract-provider/api/v1/externalevents/configuration \
     ...
     -d '{ "externalEventConfigurations": { "LoanBalanceChangedBusinessEvent": false } }'
```
