...
- The **`BulkImportApiResource`** REST resource exposes `GET /v1/imports?entityType=<code>` to list previous imports and a download endpoint for the rendered template.
...
The dispatch matrix in `BulkImportWorkbookPopulatorServiceImpl.getTemplate(...)` and `BulkImportWorkbookServiceImpl.importWorkbook(...)` is a long `if (entityType.equalsIgnoreCase(GlobalEntityType.X.toString()))` chain — see [Import handlers](/bulkimport/import-handlers) and [Populators](/bulkimport/populators) for the per-entity classes.
...
sequenceDiagram
    ...
    EntityResource-->>Operator: 200 { importId }
    EntityResource->>Listener: publish BulkImportEvent
    ...
    Listener->>Listener: update ImportDocument totals
...
The populator service today always writes `HSSFWorkbook` (`.xls`), but the upload side accepts all three formats via `ImportFormatType.of(...)`.
...
Uploads do not block the calling HTTP thread for long-running imports. `BulkImportEventListener` is annotated `@EventListener` and listens for `BulkImportEvent` (in `data/BulkImportEvent.java`); the entity resource publishes the event after persisting the workbook bytes and immediately returns the new `importDocumentId`. The listener resolves the right `ImportHandler` by `GlobalEntityType` and invokes `process(workbook, locale, dateFormat)`.

<Tip>
Because the import runs in a Spring listener and not in the original HTTP transaction, every row that ends up calling `PortfolioCommandSourceWritePlatformService.logCommandSource(...)` gets its own command audit trail in `m_portfolio_command_source`. Bulk import is therefore fully reproducible — replaying the same command stream from the audit table produces the same end state.
</Tip>
