...
<Tip>
The populator service stamps `application/vnd.ms-excel` (`.xls`) headers because it uses `HSSFWorkbook`. The upload side currently restricts itself to the same format — `BulkImportWorkbookServiceImpl.importWorkbook(...)` runs Apache Tika on the byte stream and rejects anything whose detected type does not contain `msoffice` or `application/vnd.ms-excel`. The `ImportFormatType` enum lists XLSX and ODS as future targets but is not yet referenced from the upload pipeline, so re-save the template back to `.xls` before uploading.
</Tip>
