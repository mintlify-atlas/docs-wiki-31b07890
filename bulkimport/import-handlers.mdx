...
Every handler is `@Service`-registered, has package-private `readExcelFile` / `importEntity` methods, and is picked up from `BulkImportWorkbookServiceImpl.importWorkbook(...)` based on the `GlobalEntityType` code:
...
| `RECURRING_DEPOSIT_ACCOUNTS_TRANSACTIONS` (`recurringdepositstransactions`) | `importhandler/recurringdeposit/RecurringDepositTransactionImportHandler.java` | derived |
...
<Steps>
  ...
  <Step title="Wire the dispatch">
    Extend the `if/else` chain in `BulkImportWorkbookPopulatorServiceImpl.getTemplate(...)` and `BulkImportWorkbookServiceImpl.importWorkbook(...)` to pick the new populator and handler when `GlobalEntityType.X.toString()` matches.
  </Step>
</Steps>
...
<Note>
Handlers run **single-threaded** per `BulkImportEvent`. They are dispatched from `BulkImportEventListener` on Spring's default async executor, so multiple uploads can run concurrently, but rows within one workbook are processed sequentially. This matters when one row depends on another (a client referenced as a co-borrower must be created before the loan row that references them) — the operator must order rows accordingly.
</Note>
...
- Split a workbook by office and process them in parallel uploads; each upload becomes its own `BulkImportEvent` and runs on a separate listener thread.
...
<Tip>
When you re-implement an existing handler with custom business logic, override the `@Service` registration by giving your bean a higher `@Order` or a `@Primary` annotation; the dispatch chain in `BulkImportWorkbookServiceImpl.importWorkbook(...)` resolves handlers by Spring bean name through `@Autowired`, so a renamed bean is picked up automatically without touching the dispatch chain.
</Tip>
