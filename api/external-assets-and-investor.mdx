...
| POST | `/v1/external-asset-owners/transfers/{id}?command=cancel\|active` | `ExternalAssetOwnersApiResource.java` | Cancel a pending transfer or trigger buyback. |
...
| POST | `/v1/external-asset-owners/transfers/{id}?command=…` | `transferRequestWithId` | `cancel` a pending or `active` to trigger buyback. |
...
flowchart LR
  ...
  Buyback["POST /v1/external-asset-owners/transfers/{id}?command=active<br/>buyback flow"]
...
curl -k -u admin:password \
  -H 'Fineract-Platform-TenantId: default' \
  -H 'Content-Type: application/json' \
  -X POST 'https://localhost:8443/fineract-provider/api/v1/external-asset-owners/transfers/loans/911' \
  -d '{
    "ownerExternalId": "INV-0042",
    "transferExternalId": "TRF-2024-04-001",
    "settlementDate": "01 April 2024",
...
curl -k -u admin:password \
  -H 'Fineract-Platform-TenantId: default' \
  -H 'Content-Type: application/json' \
  -X POST 'https://localhost:8443/fineract-provider/api/v1/external-asset-owners/transfers/151?command=active' \
