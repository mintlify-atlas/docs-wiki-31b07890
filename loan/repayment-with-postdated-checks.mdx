...
installment the assembler moves the cheque to `PAID (2)`; when the cheque is returned
unpaid the bounce flow sets it to `BOUNCED (1)` and creates a *new* PENDING row capturing
the replacement cheque.
...
When a loan repayment posts on the installment, `RepaymentWithPostDatedChecksAssembler`
