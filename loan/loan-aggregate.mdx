...
The class is large (~1 855 lines and 100+ fields) but its state can be grouped into a handful of clusters. Each group lives in well-defined columns inside `m_loan`.
...
<CardGroup cols={2}>
  ...
  <Card title="Collateral & guarantors" icon="shield">
    `Set<LoanCollateral> collateral` and `Set<LoanCollateralManagement> loanCollateralManagements`. `Guarantor` rows live in their own table (`m_guarantor`) and reference `Loan` from the child side rather than as a collection on the aggregate root.
  </Card>
...
- **Interest recalculation** — `LoanInterestRecalculationDetails loanInterestRecalculationDetails` mirrors the per-loan settings copied from the product.
