...
Reassigns loan officers across many loans in one call (with optional date), backed by the
`assignLoanOfficersInBulk` command.
...
— base path **`/v1/loans`**, sub-paths under `/{loanId}/capitalized-incomes` and
`/{loanId}/deferredincome`.
...
— base path **`/v1/loans`**, sub-paths under `/{loanId}/buydown-fees`.
