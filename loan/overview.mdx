...
<CardGroup cols={2}>
  <Card title="fineract-loan" icon="circle-1">
    Cumulative engine. `CumulativeFlatInterestLoanScheduleGenerator`, `CumulativeDecliningBalanceInterestLoanScheduleGenerator`. Seven legacy `LoanRepaymentScheduleTransactionProcessor` implementations under `domain/transactionprocessor/impl/`.
  </Card>
...
| `domain/transactionprocessor/` | `LoanRepaymentScheduleTransactionProcessor` interface and seven `impl/` strategies (Fineract-style, RBI, Creocore, Heavens Family, Early Payment, two due-then-in-advance orderings). |
| `domain/arrears/` | Arrears aging snapshot entity. |
...
| `guarantor/` | Guarantor sub-aggregate: `Guarantor`, `GuarantorFundingDetails`, on-hold/release lifecycle. |
...
| `LoanProduct` | `fineract-loan/src/main/java/org/apache/fineract/portfolio/loanproduct/domain/LoanProduct.java` | ~1700 |
