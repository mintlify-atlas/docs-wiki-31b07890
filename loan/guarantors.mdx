...
sequenceDiagram
    ...
    Write->>Domain: assignGuarantor(fundingDetails)
    Domain->>Sav: savingsAccount.holdFunds(amount)
...
`GuarantorDomainServiceImpl`, which calls `SavingsAccount.holdFunds(amount)` directly to
place a *HOLD* transaction on the savings account. The resulting savings transaction is
captured by a `GuarantorFundingTransaction` row so that the hold and the funding details
stay in lock-step.
...
- **`releaseGuarantorFunds(LoanTransaction)`** is fired when the loan closes (full repayment,
  write-off or undo of the closing transaction). Each `ACTIVE` `GuarantorFundingDetails` is
  moved to `COMPLETED` and the hold on the savings account is released, with a release-type
  `GuarantorFundingTransaction` recorded.
- **`transferFundsFromGuarantor(Loan)`** is fired when a write-off requires recovery from the
  guarantor. For each `ACTIVE` funding row it computes a `remainingAmount` (pro-rated against
  the guarantee total when the guarantee exceeds principal) and books an
  `AccountTransferType.LOAN_REPAYMENT` transfer from the guarantor's savings to the loan;
  `amountTransfered` and `amountRemaining` on the funding row are updated accordingly.
