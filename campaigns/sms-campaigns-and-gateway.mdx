...
| Method | Path | Purpose |
| ------ | ---- | ------- |
| GET    | `/v1/sms` | List all outbound messages (`List<SmsData>`) |
| POST   | `/v1/sms` | Manually enqueue a message |
| GET    | `/v1/sms/{resourceId}` | Retrieve one |
| GET    | `/v1/sms/{campaignId}/messageByStatus` | Paginated per-campaign drill-down filtered by `status`, `fromDate`, `toDate` |
...
`c_external_service_properties` joined to the `c_external_service`
row named `MESSAGE_GATEWAY` (read by
`ExternalServicesPropertiesReadPlatformService.getSMSGateway()`).

The property keys read out of
`ExternalServicesConstants` are:

| Property key | Field on `MessageGatewayConfigurationData` |
| ------------ | ------------------------------------------ |
| `host_name`     | `hostName()` — gateway host |
| `port_number`   | `portNumber()` — gateway port |
| `end_point`     | `endPoint()` — base path under host |
| `tenant_app_key`| `tenantAppKey()` — sent as `Fineract-Tenant-App-Key` |

`SmsConfigUtils` then builds the request:
...
(`infrastructure/sms/domain/SmsMessageStatusType.java`):

| Value | Constant |
| ----- | -------- |
| 0     | `INVALID` |
| 100   | `PENDING` |
| 150   | `WAITING_FOR_DELIVERY_REPORT` |
| 200   | `SENT` |
| 300   | `DELIVERED` |
| 400   | `FAILED` |
...
`PUT /v1/sms/{id}` only updates the message body (`SmsUpdateRequest`
carries just a `message` field). To re-attempt delivery, reset
`status_enum` to `100` (`PENDING`) directly in
`sms_messages_outbound` — the next tick of
`SEND_MESSAGES_TO_SMS_GATEWAY` will pick it up.
</Accordion>
