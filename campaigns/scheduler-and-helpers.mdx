...
external SMS gateway (configured per-tenant in `c_external_service`)
and any tasklet that needs to talk HTTP to it.
...
- Reads the per-tenant gateway configuration (hostname, port, base
  path, tenant app key) from `c_external_service`.
...
flowchart LR
  ...
  Props --> CES[(c_external_service<br/>SMS_GATEWAY)]
...
```mermaid
flowchart TB
  subgraph Scheduler tick - Quartz / Cron
