...
campaign FK, the status enum (`EmailMessageStatusType` —
`PENDING`, `SENT`, `FAILED`), and the timestamps the read services use
to surface "pending" / "sent" / "failed" listings.
...
A small endpoint surfacing the SMTP host, port, from-address, username,
password and "use STARTTLS" flag stored as `c_external_service` rows
with `external_service_name = 'SMTP_EMAIL_ACCOUNT'`. The
`EmailConfigurationWritePlatformServiceImpl` persists changes; the
`EmailMessageJobEmailServiceImpl` reads them at send time so a hot
configuration change is picked up on the next job tick without a
restart.
...
1. Reads the SMTP configuration via `EmailConfigurationReadPlatformService`.
2. Builds a `JavaMailSenderImpl` on the fly (host, port, username,
   password, STARTTLS).
3. Creates a `MimeMessage`, attaches the optional file (PDF/XLS/CSV
   produced by the stretchy report), sets `from`/`to`/`subject`/`body`
   and calls `JavaMailSenderImpl.send(...)`.
4. Catches `MailException` / `MessagingException` and flips the row to
   `FAILED` with the exception message, or to `SENT` on success.
...
`EmailMessageStatusType`: `PENDING`, `SENT`, `FAILED`.

`ScheduledEmailAttachmentFileFormat`: enumerates `PDF`, `XLS`, `XLSX`,
`CSV` — what the stretchy report should render as.
...
PATCH the row via `PUT /v1/email/{id}` and reset `status_enum` to
`PENDING`. The next `EXECUTE_EMAIL` tick picks it up. The `failedEmail`
listing is the easiest way to find candidates.
</Accordion>
