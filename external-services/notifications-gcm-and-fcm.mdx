...
There is no `/v1/notifications` resource. To enqueue a push, insert a row into `sms_messages_outbound` with `is_notification = 1`. The request DTO `SmsCreationRequest` is intentionally narrow — it does **not** expose an `isNotification` flag:
...
`SmsMessageAssembler.assembleFromJson(...)` only sets `isNotification = true` when the request carries a `campaignId` that resolves to a `SmsCampaign` with `isNotification()` true. In practice that gives you two ways to produce a push:

1. **Reference an existing notification campaign.** Create a `SmsCampaign` with notification mode true, then `POST /v1/sms` referencing its `campaignId`. The flag propagates to the row.
2. **Bypass the API.** Insert directly into `sms_messages_outbound` with `is_notification = 1` from a downstream service or migration script — the scheduler will pick it up the same way.
