...
Note the `password` is returned as `XXXX` — see [Secret masking](#secret-masking).
...
- **The mask is non‑reversible.** `StringUtil.maskValue` substitutes a fixed `XXXX` string. The round‑trip behaviour in `update(...)` exists precisely so a UI can re‑PUT the masked value without nuking the password.
...
|---|---|---|
| Document storage | `S3ContentRepository` | `s3_bucket_name`, `s3_access_key`, `s3_secret_key` |
