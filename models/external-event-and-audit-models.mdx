...
### `ExternalEventView` (projection interface)

- **File:** `fineract-core/src/main/java/org/apache/fineract/infrastructure/event/external/repository/domain/ExternalEventView.java`
- **Type:** Spring Data projection interface (not a database view) — exposes a read-only subset of `m_external_event` columns to the publisher pipeline so the full byte[] payload doesn't have to be hydrated for every consumer query.
- **Important fields:** `Long getId()`, `String getType()`, `String getCategory()`, `String getSchema()`, `byte[] getData()`, `OffsetDateTime getCreatedAt()`, `ExternalEventStatus getStatus()`, `OffsetDateTime getSentAt()`, `String getIdempotencyKey()`, `LocalDate getBusinessDate()`, `Long getAggregateRootId()`.
- **Key relationships:** Read-only; backed by `m_external_event`.
