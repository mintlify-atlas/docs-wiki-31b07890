...
flowchart LR
    ...
    Disc -.->|fallback for cumulative| CumImpl[CumulativePossibleNextRepaymentCalculationServiceImpl]
...
|--------------------------------------------------|----------------------------------------------------------------|--------------------------------------------------------------|
| Framework class                                  | `AbstractPossibleNextRepaymentCalculationService`              | Same — extended                                              |
| Routing key                                      | Default — `canAccept` returns `true` when not progressive      | `loan.isProgressiveSchedule()`                                |
| Future outstanding source                        | Re-runs cumulative interest accrual against the installments   | Loads the persisted `ProgressiveLoanModel` snapshot           |
...
| EMI recomputation per recalc                     | Time-bucketed accrual                                          | `recalculateModelOverdueAmountsTillDate(...)` + period read   |
| Fees/penalties                                   | Same — read from `LoanRepaymentScheduleInstallment`            | Same                                                          |

The shared abstract class ensures both engines apply the **same five guard conditions** before they bother
to recompute — interest recalculation must be enabled, rest frequency must not match the repayment
frequency, the loan must not be charged off as of the date, the next due date must be strictly in the
future, the next installment must not start today, and the loan must have a positive disbursed amount.
Inside those guards, the progressive engine substitutes its snapshot-driven recomputation for the
cumulative engine's accrual replay.
