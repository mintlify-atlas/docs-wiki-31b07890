...
A semantic validator also runs inside the entity:

```java
// SavingsProduct.java :: validateInterestPostingAndCompoundingPeriodTypes(...)
// Rejects e.g. monthly compounding + daily posting.
```

This guarantees the invariants that the [interest engine](/savings/interest-posting-and-compounding) relies on.
