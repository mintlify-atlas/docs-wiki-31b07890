...
| 13 | `SAVINGS_NO_ACTIVITY_FEE` | Fires when the no-activity threshold is reached. |
...
So a `PERCENT_OF_AMOUNT` withdrawal fee is computed every time it fires — `populateDerivedFields(withdrawalAmount, percentage)` is called from `updateWithdrawalFeeAmount(...)` immediately before the fee is paid.
...
sequenceDiagram
  ...
  SA->>Charge: updateWithdrawalFeeAmount(withdrawalAmount)
