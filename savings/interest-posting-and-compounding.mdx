...
You can compound daily but post quarterly. You cannot post more often than you compound — `FixedDepositProduct.validateInterestPostingAndCompoundingPeriodTypes(...)` (and the matching method on `SavingsAccount`) enforces this. The actual enums:
...
Two semantic invariants are enforced inside `FixedDepositProduct.validateInterestPostingAndCompoundingPeriodTypes(...)` (mirrored on `SavingsAccount`) and reused by FD/RD:
