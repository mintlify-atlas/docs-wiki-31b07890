...
The enum-styled name `WC_BREACH_SCHEDULE` and human-readable `WC Breach Schedule` are what the COB ordering configuration uses — they appear in `m_batch_business_steps` and in the operator UI ordering view.
...
In tenant configuration the step is typically disabled (left out of `m_batch_business_steps`); enabling it adds a measurable log line per loan and is useful for verifying the pipeline is alive.

## Recommended order

Although the actual order is configured per-tenant via `m_batch_business_steps`, the steps make most sense run in this sequence:
...
3. **Has a unique `getEnumStyledName()`.** That value lives in `m_batch_business_steps` (the per-tenant ordering table) and is keyed unique per job.

Adding the row to `m_batch_business_steps` activates the step in the chain. The COB pipeline will then execute it in the configured order — no other code change is needed.
