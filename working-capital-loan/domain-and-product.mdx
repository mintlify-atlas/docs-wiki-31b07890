...
`WorkingCapitalAccountingRuleType` is a small enum with two values today — `NONE` (`"wcAccountingRuleType.none"`, "No accounting") and `CASH_BASED` (`"wcAccountingRuleType.cash"`, "Cash based accounting"). The selection drives `WorkingCapitalLoanAccountingProcessor` (in `accounting/`).
