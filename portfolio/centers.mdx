...
sequenceDiagram
  ...
  participant H as SaveCenterCollectionSheetCommandHandler
  ...
  API-->>LO: JSON preview
  LO->>API: POST /v1/centers/{id}?command=saveCollectionSheet (adjusted amounts)
  API->>H: processCommand(JsonCommand)
  H-->>API: CommandProcessingResult (loan repayments + savings deposits booked)
```
