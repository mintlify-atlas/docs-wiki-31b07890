...
Roles can be disabled via `POST /v1/roles/{roleId}?command=disable` (sets `is_disabled = true`). `AppUser.populateGrantedAuthorities()` does **not** filter by `is_disabled` and the role-assignment path in `RoleWritePlatformServiceJpaRepositoryImpl` does not currently reject disabled roles (the check is present but commented out at line ~230). To make a disable take effect for active users, also detach the role from them via `PUT /v1/users/{userId}`.
...
private void validateHasPermission(final String prefix, final String resourceType) {
    final String authorizationMessage = "User has no authority to " + prefix + " " + resourceType.toLowerCase(Locale.ROOT) + "s";
...
The same `hasNotPermissionForAnyOf("ALL_FUNCTIONS", "ALL_FUNCTIONS_READ", matchPermission)` check is used for every action — so at the resource layer both wildcards satisfy reads and writes. The narrower `ALL_FUNCTIONS_WRITE` / `ALL_FUNCTIONS_READ` split is only enforced by the URL-level `hasAnyAuthority(...)` rules in `SecurityConfig` (e.g. `READ_*` routes accept `ALL_FUNCTIONS_READ`, `CREATE_*`/`UPDATE_*`/`DELETE_*` routes accept `ALL_FUNCTIONS_WRITE`).
...
| `UpdateRolePermissionsCommandHandler` | `ROLE` / `PERMISSIONS` | Bulk add/remove permissions on a role |
...
`LoginAttemptEventListener` (`fineract-provider/.../infrastructure/security/service/LoginAttemptEventListener.java`) listens on Spring Security's `AbstractAuthenticationFailureEvent` and calls `AppUser.registerFailedLoginAttempt(maxRetries)`; when the counter reaches the configured `maxLoginRetries` from `ConfigurationDomainService`, `nonlocked` is set to false. Successful authentication resets the counter via the matching `AuthenticationSuccessEvent` handler. The lockout threshold and the `is_login_retries_enabled` per-user opt-in must both be true for tracking to occur.
