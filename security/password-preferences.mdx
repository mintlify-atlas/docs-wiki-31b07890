...
A single `c_configuration` row gates time-based rotation:

| Key | Default value | Default enabled | Purpose |
| --- | ------------- | --------------- | ------- |
| `force-password-reset-days` | `0` | `false` | When enabled, passwords older than `value` days must be changed |

`ConfigurationDomainServiceJpa` exposes the row through two methods that share the same lookup:

```java
// ConfigurationDomainServiceJpa.java
public boolean isPasswordForcedResetEnable() {
    return getGlobalConfigurationPropertyData(
        GlobalConfigurationConstants.FORCE_PASSWORD_RESET_DAYS).isEnabled();
}
public Long retrievePasswordLiveTime() {
    return getGlobalConfigurationPropertyData(
        GlobalConfigurationConstants.FORCE_PASSWORD_RESET_DAYS).getValue();
}
```
