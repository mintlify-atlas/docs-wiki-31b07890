...
```mermaid
flowchart LR
    A[/api/**, /oauth2/**, /login, /swagger-ui, /actuator/**/] --> B{Order 1 publicEndpoints}
...
<Warning>
The OAuth2 chain disables CSRF (`csrf(AbstractHttpConfigurer::disable)`) and `setEraseCredentialsAfterAuthentication(false)`. Both are deliberate — CSRF protection is unnecessary for stateless JWT APIs and the credentials are retained so `TwoFactorAuthenticationFilter` can rebuild the authentication object with the extra authority.
</Warning>
